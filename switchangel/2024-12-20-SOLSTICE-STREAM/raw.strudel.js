...
const arp = note(chords[0]).arp("{0 1 2 3 2 1}%16")
  ...
  .grab("e:b:g:a:c:d:f#").room(1).z1("5:8")
...
const chordarp = note("<g3 g2>/4").s("morrow").begin(saw.segment(16).range(.2, .5)).slow(.5)
  .clip(.8)
  .struct("x(11,16)").chorus(.5).coarse(12)
  .segment(16).postgain(.8).pan(rand);

const hypelead = note("{[f#] g a [[d e] | e] [~ | g] [f#] [[a g] | g | g]}")
  .add(12).add(tran).grab("e:b:g:a:c:d:f#")
  .s("tear").begin(rand.range(0, 0.3)).clip(.6).delay(.7).delay(.7).delayt(.7)
  .postgain(.61).shimmer(.7).room(1).phaser(.5).roomsize(.9).gain("[.9 | 1]");
...
// d4: hypelead.orbit(4)
d5: shell.orbit(5);
d6: arp.orbit(6).roomsize(slider(1, 0, 1));
d7: chordarp.orbit(6);
